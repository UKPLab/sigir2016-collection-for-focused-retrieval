...
public class Mathematics {
    ...
    public static double log2(double x) {
        return naturalLogToBase2Log(Math.log(x));
    }
    ...
    public static double log2Gamma(double z) {
        if (z < 0.5) {
            return Mathematics.log2(Math.PI)
                - Mathematics.log2(Math.sin(Math.PI * z))
                - log2Gamma(1.0 - z);
        }
        double result = 0.0;
        while (z > 1.5) {
     ...
     * the java constant {@link Math#E}.
     */
    public static final double LOG2_E = log2(Math.E);
